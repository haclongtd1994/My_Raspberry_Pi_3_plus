...
int main(void){
	...
	iret_curl_gpio = pthread_create(&thread_curl_gpio, NULL, func_thread_curl_send_data_only,(void *)"GPIO");
	...
	iret_gpio = pthread_create(&thread_gpio,NULL, func_thread_gpio,(void *)"ON");
	...
	printf("Ket qua luong dieu khien gui du lieu gpio: %d\r\n", iret_curl_gpio);
...
void *func_thread_gpio(void *ptr){
	...
	while(1){
		...
		else printf(RED"ARLET: "GRN"Data receive failed pleave check\r\n"RESET);
	}
}
void *func_thread_uart_transmit(void *ptr){
	int ret, fd;
