...
void *func_thread_dht11(void *ptr){
	while(1){
		...
		while(check1==0||check2==0) pthread_cond_broadcast(&dht11_cond);
	}
}
...
void *func_thread_curl_dht11_humidity(void *ptr){
	while(1){
		pthread_mutex_lock(&dht11_mutex);
		while(check1==1) pthread_cond_wait(&dht11_cond,&dht11_mutex);
...
void *func_thread_curl_dht11_temperature(void *ptr){
	...
	while(1){
		pthread_mutex_lock(&dht11_mutex);
		while(check2==1) pthread_cond_wait(&dht11_cond,&dht11_mutex);
